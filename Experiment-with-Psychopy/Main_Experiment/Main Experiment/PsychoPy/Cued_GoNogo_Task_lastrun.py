...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='H:\\Main Experiment\\Cued_GoNogo_Task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    fixation = visual.TextStim(win=win, name='fixation',
        text='Welcome to the Final Experiment!\n\nThank you for participating in the scan.\n\nPressing the SPACEBAR will sart the scan.\n',
        font='NanumSquare',
    ...
    try:
        ...
        info = StreamInfo('Markers', 'Markers', 1, 0, 'string', 'psychopygonogo123')
        outlet = StreamOutlet(info)
        outlet.push_sample(['ExperimentStart'])
        using_lsl = True
        print("LSL marker stream created successfully")
    ...
    BlockStart_Msg = visual.TextStim(win=win, name='BlockStart_Msg',
        text='',
        font='NanumSquare',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
    ...
    BlockEnd_Msg = visual.TextStim(win=win, name='BlockEnd_Msg',
        text='',
        font='NanumSquare',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
    ...
    Blocks = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('block/BlockList.xlsx'), 
        seed=None, 
    )
    ...
    for thisBlock in Blocks:
        ...
        BlockStart = data.Routine(
            name='BlockStart',
            components=[BlockStart_Msg, key_resp_2],
        )
        BlockStart.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        BlockStart_Msg.setText(blockMsg)
        # create starting attributes for key_resp_2
        key_resp_2.keys = []
        key_resp_2.rt = []
        _key_resp_2_allKeys = []
        # Run 'Begin Routine' code from BreakStart_1_Marker
        send_marker("BlockStart_1")
        ...
        while continueRoutine:
            ...
            # *BlockStart_Msg* updates
            
            # if BlockStart_Msg is starting this frame...
            if BlockStart_Msg.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                BlockStart_Msg.frameNStart = frameN  # exact frame index
                BlockStart_Msg.tStart = t  # local t and not account for scr refresh
                BlockStart_Msg.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(BlockStart_Msg, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'BlockStart_Msg.started')
                # update status
                BlockStart_Msg.status = STARTED
                BlockStart_Msg.setAutoDraw(True)
            
            # if BlockStart_Msg is active this frame...
            if BlockStart_Msg.status == STARTED:
                # update params
                pass
        ...
        for thisTrial in Trials:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from FixationMarker
            send_marker('Fixation')
            ...
            Key_Resp.rt = []
            _Key_Resp_allKeys = []
            # Run 'Begin Routine' code from TrialMarkers
            cue_marker = f'Cue_{StimType}'
            ...
            while continueRoutine:
                ...
                if Key_Resp.status == STARTED and not waitOnFlip:
                    ...
                    if len(_Key_Resp_allKeys):
                        ...
                        # a response ends the routine
                        continueRoutine = False
                # Run 'Each Frame' code from TrialMarkers
                if GoNogo_Stim.status == STARTED and GoNogo_Stim.status != FINISHED:
                    if not hasattr(thisExp, 'stim_marker_sent'):
                        thisExp.stim_marker_sent = True
                        stim_marker = f'Stimulus_{StimType}'
                        send_marker(stim_marker)
                
                if Key_Resp.status == STARTED and len(Key_Resp.keys) > 0:
                    if not hasattr(thisExp, 'resp_marker_sent'):
                        thisExp.resp_marker_sent = True
            ...
            # Run 'End Routine' code from TrialMarkers
            if hasattr(thisExp, 'stim_marker_sent'):
                del thisExp.stim_marker_sent
            if hasattr(thisExp, 'resp_marker_sent'):
                del thisExp.resp_marker_sent
            ...
            if StimType == 'Go': # go cue on the condition of presenting the go cue 
                if Key_Resp.corr == 1: # if the answer is correct,
                    fb_color = 'Maroon'
                    if Key_Resp.rt is not None:
                        fb_msg = "Correct" + "\n" + "Reaction time: " + str(int(Key_Resp.rt * 1000)) + "ms"
                    else:
                        fb_msg = "Correct" + "\n" + "Reaction time: not recorded"
                else: # if correct
                    fb_color = 'MediumBlue'
                    fb_msg = "Wrong"
            elif StimType == 'NoGo': # Nogo cue on Nogo cue presentation condition 
                if Key_Resp.corr == 1: # if the answer is correct,
                    fb_color = 'Maroon'
                    fb_msg = "Correct"
                else: # if incorrect
                    fb_color = 'MediumBlue'
                    fb_msg = "Incorrect. You should not press the button."    
                    
            Feedback_Present.setColor(fb_color, colorSpace='rgb')
            Feedback_Present.setText(fb_msg)
            # Run 'Begin Routine' code from FeedbackMarker
            if hasattr(Key_Resp, 'corr'):
                if Key_Resp.corr == 1:
                    send_marker(f'Feedback_Correct_{StimType}')
                else:
                    send_marker(f'Feedback_Incorrect_{StimType}')
        ...
        BlockEnd = data.Routine(
            name='BlockEnd',
            components=[BlockEnd_Msg, key_resp_3],
        )
        BlockEnd.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        BlockEnd_Msg.setText('End of Block.\n\nTake a 4 minute break.\n\nPlease press SPACE when ready.')
        # create starting attributes for key_resp_3
        key_resp_3.keys = []
        key_resp_3.rt = []
        _key_resp_3_allKeys = []
        # Run 'Begin Routine' code from BreakEnd_1_Marker
        send_marker("BlockEnd_1")
        ...
        while continueRoutine:
            ...
            # *BlockEnd_Msg* updates
            
            # if BlockEnd_Msg is starting this frame...
            if BlockEnd_Msg.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                BlockEnd_Msg.frameNStart = frameN  # exact frame index
                BlockEnd_Msg.tStart = t  # local t and not account for scr refresh
                BlockEnd_Msg.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(BlockEnd_Msg, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'BlockEnd_Msg.started')
                # update status
                BlockEnd_Msg.status = STARTED
                BlockEnd_Msg.setAutoDraw(True)
            
            # if BlockEnd_Msg is active this frame...
            if BlockEnd_Msg.status == STARTED:
                # update params
                pass
    ...
    TaskEnd = data.Routine(
        name='TaskEnd',
        components=[ThankYou, key_resp_4],
    )
    ...
    key_resp_4.rt = []
    _key_resp_4_allKeys = []
    # Run 'Begin Routine' code from EndMarker
    send_marker('ExperimentEnd')
